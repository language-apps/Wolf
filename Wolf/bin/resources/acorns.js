...
function Toolbar(dictionaryObject)
{
	...
	this.getLanguageName = function()           { return langFonts[primaryLang].name; }
...
function Dictionary(parsedObject)
{
	...
	this.loadDictionary = function(doSort, pattern)
	{
		...
		var newWordList, html, list, i, listLen, searchType;
		...
		var searchElement = document.getElementById("searchSelect");
		searchElement.onchange = searchTypeHandler;
		...
		{
			searchElement = document.getElementById("searchSelect");
			searchType = searchElement.value;
	...
	var searchTypeHandler = function(e)
	{
		...
		var language = toolbar.getLanguageName();
		var handler;
		
		if (searchType == 'word')
		{
		    handler = new KeyboardHandler(searchText, language);
		}
		else
		{
			handler = new KeyboardHandler(searchText, "");
	    }
	}
	
	/** Private Method: Search for matching words, categories, or ontology 
	 *
	 * @param type word, category, Main Entry, or ontology
	...
	{
		var i, j,k, text, newText, title, elements, values, added, translations;
		...
		for (i = 0; i < len; i++)
		{
			switch (type.toLowerCase())
			{
				case "word":
					...
					if (!translations || translations.length==0 || !translations[0].firstChild) 
						continue;
				...
				case "ontology":
					...
					for (j=0; j<eLen; j++)
					{
						text = elements[j].innerHTML;
						if (!text || text.length==0)
						{
							text = elements[j].getAttribute("child");
				...
				case "category":
					...
					for (j=0; j<eLen; j++)
					{
						...
						if (title && title.toLowerCase() == categorySearch)
						{
							text = elements[j].textContent;
							const splitText = text.split("http");
							if (splitText.length>0) text = splitText[0];										
							if (!text) continue;
