...
function Format(sys, doc)
{
	...
	var styleDefinitions = function(build, definitions, template, languages)
	{
		...
		var node, hdrName = template[t.D_HDR.name];
		
		// Index where definition should appear among the column output
		if (!hdrName || hdrName.length == 0)
		{
		    hdrName = template[t.S_HDR.name];
			if (!hdrName || hdrName.length == 0) return;
		}
		...
		var order = hdrName.count;
		var rows = hdrName.rows;
		var title = hdrName.title;
		if (title) title = title.replace(/\n/g, "<br>\n");
		if (title) title = title.replace(/\t/g, "&#09");
		if (title) title = title.replace(/\s/g, "&nbsp");

		var format = hdrName.format;
		var separator = hdrName.separator;
		var showPicture = hdrName.picture;
		var show = showPicture && showPicture=="true";
		var	isField;
		...
		for (i=0; i<len; i++)
		{
			...
			styleFields(build, elementLists[COLUMNS], template[t.D_HDR.name][t.D_FLD.name], template[t.S_HDR.name][t.S_FLD.name], languages, index, text, show);
			styleRows(build, elementLists[ROWS], template, languages);
